...
extern "C" {
  static PyObject * cpmedian(PyObject *self, PyObject *args, PyObject *keywds);

  static PyMethodDef module_methods[] = {
    {"cpmedian", (PyCFunction)cpmedian, METH_VARARGS | METH_KEYWORDS, "Calculate median using cp select method"},
  ...
  static struct PyModuleDef moduledef = {
      ...
      "cp_select",     /* m_name */
      "Module with CP select median algorithm",  /* m_doc */
      -1,                  /* m_size */
...
#if PY_MAJOR_VERSION < 3
    PyMODINIT_FUNC initcp_select(void) {
        PyObject* m;
        m = Py_InitModule3("cp_select", module_methods, "Example module that creates an extension type.");
        if (m == NULL){ return; }
        import_array(); /* required NumPy initialization */
    }
#else
    PyMODINIT_FUNC PyInit_cp_select(void) {
        PyObject* m;
...
template<typename InputIterator, typename Tbase, typename Tsum>
	void ComputeMaxMinSum(InputIterator first, InputIterator last, Tbase & maxA, Tbase & minA, Tsum & sum)
	{
	  ...
	  maxminsum<const thrust::tuple<Tbase,Tbase,Tsum> > binaryop;
...
*/	
	...
	double  median_min_double( thrust::host_vector<double> &D,  int n, int CPiters,  int method, int k=0)
	{
        ...
        int median_min_int( thrust::host_vector<int> &D,      int n, int CPiters,  int method,  int k=0)
        {
                typedef float T;
                thrust::device_vector<T> DataD = D;
                T res=median_min(DataD.begin(), DataD.end(),    CPiters,   method,k);
        ...
        long median_min_long( thrust::host_vector<long> &D,      int n, int CPiters,  int method,  int k=0)
        {
                typedef float T;
                thrust::device_vector<T> DataD = D;
                T res=median_min(DataD.begin(), DataD.end(),    CPiters,   method,k);
  ...
  static PyObject * cpmedian(PyObject *self, PyObject *args, PyObject *keywds) {
	...
	int i = 0;
        if (nonzero && lthresh == INT_MIN && hthresh == INT_MAX) {
          //only nonzero
          for (int j = 0; j < n; j++) {
            if (data[j] != 0) h_x[i++] = data[j];
          }
        } else if (!nonzero && lthresh != INT_MIN && hthresh == INT_MAX) {
          //only lthresh
          for (int j = 0; j < n; j++) {
            if (data[j] >= lthresh) h_x[i++] = data[j];
          }
        } else if (!nonzero && lthresh == INT_MIN && hthresh != INT_MAX) {
          //only hthresh
          for (int j = 0; j < n; j++) {
            if (data[j] <= hthresh) h_x[i++] = data[j];
          }
        } else if (nonzero && lthresh != INT_MIN && hthresh == INT_MAX) {
          //nonzero and lthresh 
          for (int j = 0; j < n; j++) {
            if (data[j] != 0 && data[j] >= lthresh) h_x[i++] = data[j];
          }
        } else if (nonzero && lthresh == INT_MIN && hthresh != INT_MAX) {
          //nonzero and hthresh
          for (int j = 0; j < n; j++) {
            if (data[j] != 0 && data[j] <= hthresh) h_x[i++] = data[j];
          }
        } else if (!nonzero && lthresh != INT_MIN && hthresh != INT_MAX) {
          //lthresh and hthresh
          for (int j = 0; j < n; j++) {
            if (data[j] >= lthresh && data[j] <= hthresh) h_x[i++] = data[j];
          }
        } else if (nonzero && lthresh != INT_MIN && hthresh != INT_MAX) {
          //all three
          for (int j = 0; j < n; j++) {
            if (data[j] != 0 && data[j] >= lthresh && data[j] <= hthresh) h_x[i++] = data[j];
          }
        }
        n = i;
	h_x.resize(i);
	...
	med = (double)median_min_float(h_x, n, 7, method, k+1);
      ...
      if (nonzero || lthresh != INT_MIN || hthresh != INT_MAX) {
        ...
        h_x.resize(i);
        k = (n-nhigh-nlow)/2+nlow;
        med = (double)median_min_long(h_x, n, 7, method, k+1);
      } else {
        k = (n-nhigh-nlow)/2+nlow;
        thrust::host_vector<long> h_x(data, data+n);
        med = (double)median_min_long(h_x, n, 7, method, k+1);
      }
    } else {
