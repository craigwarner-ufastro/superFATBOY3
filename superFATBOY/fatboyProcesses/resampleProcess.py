    ...
    def resampleData(self, fdu, calibs):
        ...
        if (fdu.dispersion == fdu.DISPERSION_HORIZONTAL):
            ...
            if (slitmask is not None):
                ...
                smData = np.ascontiguousarray(smData[:ysize, :])  # Remove extraneous top row
                if (doSlitmask):
            ...
            if ('cleanSky' in calibs and not calibs['cleanSky'].hasProperty("is_resampled")):
                ...
                data = np.ascontiguousarray(data[:ysize, :])  # Remove extraneous top row
            ...
            if (fdu.hasProperty("cleanFrame")):
                ...
                cleanData = np.ascontiguousarray(cleanData[:ysize, :])  # Remove extraneous top row
            ...
            if (fdu.hasProperty("noisemap")):
                ...
                nmData = np.sqrt(nmData)
                expmap[expmap == 0] = 1
                nmData /= expmap
                nmData = np.ascontiguousarray(nmData[:ysize, :])  # Remove extraneous top row
            ...
            if (fdu.hasProperty("exposure_map")):
                ...
                expData = np.ascontiguousarray(expData[:ysize, :])  # Remove extraneous top row
                # Update "exposure_map" data tag
                fdu.tagDataAs("exposure_map", data=np.sqrt(expData))
            ...
            resampData = np.ascontiguousarray(resampData[:ysize, :])  # Remove extraneous top row
            # Update data
