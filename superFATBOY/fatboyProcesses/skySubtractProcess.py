...
class skySubtractProcess(fatboyProcess):
    ...
    def createRoughSkyAndSextractedImage(self, fdu, prevProc=None):
        ...
        if (sxtMethod == "sep"):
            self.createSepImage(fdu, data)
        else:
            tempSSfile = self._fdb._tempdir+"/TEMPss_"+fdu.getFullId()
            write_fits_file(tempSSfile, data, data.dtype, overwrite=True, log=self._log)
            self.createSextractedImage(fdu, tempSSfile)
        del data
    ...
    def createSextractedImage(self, fdu, tempSSfile=None):
        ...
        try:
            ...
            objMask = np.zeros(fdu._shape, np.int32)
        ...
        if (twoPass == "yes"):
            ...
            if (self._fdb.getGPUMode()):
                ...
                objMask = np.where(objMask > two_pass_reject_level, 1, 0)
                data *= objMask
    ...
    def createSepImage(self, fdu, ssdata):
        ...
        thresh = sep_thresh*bkg.globalrms
        bkg.subfrom(ssdata)
        try:
            ...
            try:
                ...
                fdu.tagDataAs("sepMask", np.zeros(ssdata.shape, bool))
        ...
        objMask = np.zeros(ssdata.shape, bool)
    ...
    def createMasterSkyOffsource(self, fdu, skies, skymethod, msname, msfilename):
        ...
        for skyfdu in skies:
            ...
            if (sxtMethod == "sep"):
                self.createSepImage(skyfdu, data)
            else:
                tempSSfile = self._fdb._tempdir+"/TEMPss_"+skyfdu.getFullId()
                write_fits_file(tempSSfile, data, data.dtype, overwrite=True, log=self._log)
                self.createSextractedImage(skyfdu, tempSSfile)
    ...
    def extObjSky(self, image, msky):
        ...
        for j in range(10):
            for l in range(10):
                ...
                if len(temp[np.where(temp != 0)] != 0):
                    sds[j*10+l] = temp[np.where(temp != 0)].std()
                else:
                    sds[j*10+l] = 0.
        stddev = gpu_arraymedian(sds, kernel=kernel)

        good = np.logical_and(ss_image < 5*stddev, ss_image != 0)
        immed = gpu_arraymedian(image[good], kernel=kernel)
        change = 0
        while (change < 0.99 or change > 1.01):
            oldGood = good
            stddev = ss_image[np.where(ss_image != 0)].std()
            ss_image = image-(immed/msmed)*msky
            good = np.logical_and(ss_image < 5*stddev, ss_image != 0)
            immed = gpu_arraymedian(image[good], kernel=kernel)
    ...
    def findSkySubtractMethods(self, fdu):
        ...
        if (skymethod.lower() in self.ssmethods):
            ...
            for fdu_obj in self._fdb.getFDUs(tag=fdu.getTag()):
                if (not fdu_obj.hasProperty("sky_method")):
                    fdu_obj.setProperty("sky_method", skymethod)
            ...
            for j in range(len(methodList)-1, -1, -1):
                ...
                if (not methodList[j][1] in self.ssmethods):
                    print("skySubtractProcess::findSkySubtractMethods> Warning: line "+str(j)+" misformatted in "+skymethod)
                    self._log.writeLog(__name__, " line "+str(j)+" misformatted in "+skymethod, type=fatboyLog.WARNING)
                    methodList.pop(j)
                    continue
            for fdu_obj in self._fdb.getFDUs(tag=fdu.getTag()):
                if (fdu_obj.hasProperty("sky_method")):
                    continue
                for method in methodList:
                    if (method[0].lower() == fdu_obj.filter.lower()):
                        fdu_obj.setProperty("sky_method", method[1])
                    elif (len(method[0]) > 2 and fdu_obj._id.lower().find(method[0].lower()) != -1):
                        fdu_obj.setProperty("sky_method", method[1])
        else:
    ...
    def fitSkySubtractedSurf(self, fdu):
        ...
        if (sxtMethod == "sep"):
            if (not fdu.hasProperty("sepMask")):
                self.createSepImage(fdu, fdu.getData().copy())
            objMask = fdu.getData(tag="sepMask")
        else:
            sxtfile = self._fdb._tempdir+"/TEMPsxt_"+fdu.getFullId()
    ...
    def setDefaultOptions(self):
        ...
        self._options.setdefault('sky_subtract_method', "remove_objects")
        ...
        self._options.setdefault('onsource_sorting_key', 'full')
        ...
        self._options.setdefault('source_extract_method', 'sep')
