...
def bpm_replace_linterp_x(data, bpm=None, niter=1, arg=None):
    ...
    #Set np.zeros not flagged as bad pixels to be 1.e-6 and all bad pixels to be zero
    z = (bpm == 0)*(data == 0)
    data[z] = 1.e-6
    data[bpm] = 0
...
def bpm_replace_median_neighbor_gpu(data, bpm=None, niter=1, arg=None):
    ...
    out_bpm = np.empty(data.shape, bool)
    rows = data.shape[0]
    cols = data.shape[1]
    ct = np.zeros(1, np.int32)
...
def bpm_replace_scipy_interp2d(data, bpm=None, niter=1, arg=None):
    ...
    for iter in range(niter):
        ...
        for j in range(len(b[0])):
            ...
            tempy = np.array(tempy, dtype=np.float64)
            tempdata = np.array(tempdata, dtype=np.float64)
        ...
        for j in range(len(b[0])):
            ...
            for i in range(ymin, ymax):
                for k in range(xmin, xmax):
                    ...
                    #Check if data == 0 instead of if bpm != 1 because on iteration 2
                    #bpm is still 1 on data that has been replaced on iteration 1
                    #Don't use data value 1.e-6 that were np.zeros for interpolation either
            ...
            try:
                newdata[y,x] = griddata(np.array([tempx, tempy]).transpose(), tempdata, (x,y), method='cubic', fill_value=fv)
                nreplace += 1
            except Exception as ex:
                print(ex)
                continue
...
def bpm_replace_weighted(data, bpm=None, niter=1, arg=None):
    kernel = np.array([[0,2,0],[0,5,0],[1,0,1],[0,5,0],[0,2,0]])
    if (arg is not None):
        #Validate that this is np.array([...]) before using eval
        if (arg.startswith("np.array([") and arg.endswith("])")):
            kernel = eval(arg)
...
class BadPixelMaskSpecProcess(fatboyProcess):
    ...
    def createBadPixelMask(self, fdu, sourceFDU, calibs):
        ...
        if (self._fdb.getGPUMode()):
            if (fdu._specmode == fdu.FDU_TYPE_LONGSLIT):
                ...
                data = cp.empty(fdu.getShape(), np.int32)
                blocks = data.size//block_size
                if (data.size % 512 != 0):
                    blocks += 1
                gpu_bad_pixel_mask((blocks,1), (block_size,1,1), (data, cp.asarray(sourceFDU.getData()), np.int32(nx), np.int32(ny), np.float32(lo), np.float32(hi), np.int32(edge_reject), np.float32(radius_reject)))
            else:
                ...
                if (data.size % 512 != 0):
                    blocks += 1
                gpu_bad_pixel_mask_mos((blocks,1), (block_size,1,1), (data, cp.asarray(sourceFDU.getData()), cp.asarray(calibs['slitmask'].getData().astype(np.int32)), np.int32(nx), np.int32(ny), cp.asarray(lo), cp.asarray(hi), np.int32(edge_reject), np.float32(radius_reject)))
            #Convert to bool here
            data = data.get().astype(bool)
        else:
            if (fdu._specmode == fdu.FDU_TYPE_LONGSLIT):
                if (doNormalize == "yes" and not sourceFDU.hasProperty("normalized")):
                    #need to normalize source first
                ...
                #Initialize as all np.zeros
                data = np.zeros(sourceFDU.getData().shape, bool)
                for j in range(calibs['nslits']):
                    slit = calibs['slitmask'].getData() == (j+1)
                    if (clippingMethod == "values"):
                        lo = clipping_low
            ...
            if (radius_reject > 0):
                (nx, ny) = data.shape
                xs = np.arange(ny*nx).reshape(ny,nx) % nx - ((nx-1)/2.)
                ys = np.arange(ny*nx).reshape(ny,nx) // nx - ((ny-1)/2.)
    ...
    def execute(self, fdu, prevProc=None):
        ...
        if (self.getOption("behavior", fdu.getTag()).lower() == "interpolate"):
            ...
            t = time.time()
            (data, nreplace) = bpm_replace_algorithm(fdu.getData().astype(np.float32), badPixelMask.getData(), niter, arg)
            t2 = time.time()-t
