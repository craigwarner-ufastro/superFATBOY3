...
class sinfoniBadLineRemovalProcess(fatboyProcess):
    ...
    def removeBadLines(self, fdu):
        ...
        mask_back = np.zeros(fdu.getShape(), dtype=np.int32)
        ...
        bad = np.abs(diffbackpix) > nsigmaback * sigmaback
        nbad = np.sum(bad)
        ...
        ybad = ybackpix[bad]
        good = np.abs(diffbackpix) <= nsigmaback * sigmaback
        ...
        for k in range(nbad):
            ...
            if (yval != yprev):
                ...
                kline_mean = np.mean(np.array(kline))
