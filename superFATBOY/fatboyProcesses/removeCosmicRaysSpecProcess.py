...
from superFATBOY.fatboyProcesses.badPixelMaskSpecProcess import (
    bpm_replace_median_neighbor, bpm_replace_median_neighbor_gpu)
...
class removeCosmicRaysSpecProcess(fatboyProcess):
    ...
    def removeStrayLight(self, fdu, calibs):
        ...
        # Create objmask
        objmask = np.zeros(data.shape, dtype=bool)
        ...
        if (sl_method == "mask"):
            ...
            npix = objmask.sum()
            data = fdu.getData().astype(np.float32)  # re-copy data
        ...
        if (self.getOption('write_calib_output', fdu.getTag()).lower() == "yes"):
            # write out sl data np.array - 0 np.where no stray light found, actual value assigned to sl otherwise
    ...
    def runDcr(self, fdu, calibs):
        ...
        if (self.getOption('write_calib_output', fdu.getTag()).lower() == "yes"):
            # write out cr data np.array - 0 np.where no cosmic rays found, actual value assigned to cr otherwise
            # make directory if necessary
            outdir = str(self._fdb.getParam("outputdir", fdu.getTag()))
            if (not os.access(outdir + "/removedCosmicRays", os.F_OK)):
                os.mkdir(outdir + "/removedCosmicRays", 0o755)
            ...
            for j in range(nslits):
                ...
                if (fdu.dispersion == fdu.DISPERSION_HORIZONTAL):
                    ...
                    if (inpaint):
                        ...
                        cleanData[ylo:yhi, :][tempMask] = c[tempMask].astype(np.float32)
                    else:
                    ...
                    if (inpaint):
                        m, c = mdl.clean(slit, threshold=thresh, inpaint=inpaint, segment=True)
                        cleanData[:, xlo:xhi][tempMask] = c[tempMask].astype(np.float32)
                    else:
                        m = mdl.clean(slit, threshold=thresh, inpaint=inpaint, segment=True)
